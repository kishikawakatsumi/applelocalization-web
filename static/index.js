...
function init(platform) {
  ...
  const table = new Tabulator("#table", {
    ...
    ajaxResponse: function (url, params, response) {
      ...
      const textContent = `${f.format(totalCount)} / ${f.format(dataCount)}`;
      document.getElementById("total-count").textContent = textContent;
  ...
  table.on("dataProcessed", () => {
    ...
    if (dataCount !== undefined) {
      const f = new Intl.NumberFormat();
      document.getElementById("data-count").textContent = `${f.format(
        dataCount
      )} /`;
    }
  });
